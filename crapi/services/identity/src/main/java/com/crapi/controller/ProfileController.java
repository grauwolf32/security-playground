...
public class ProfileController {
   ...
   * @param videoId
   * @param request
   * @return response indicating the user endpoint does not delete videos directly.
   */
   ...
   * @param videoId
   * @param request
   * @return delete the specified user video from the database and return a status message.
   */
  @DeleteMapping("/api/v2/admin/videos/{video_id}")
  public ResponseEntity<CRAPIResponse> deleteVideoAsAdmin(
      @PathVariable("video_id") Long videoId, HttpServletRequest request) {
  ...
  /**
   * @param video_id
   * @return convert video — reads the conversion param from the database and performs the
   *     operation according to that param; this endpoint is intended for local access only.
   */
